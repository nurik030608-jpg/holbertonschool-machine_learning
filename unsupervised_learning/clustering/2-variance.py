...
Provides a function to compute the sum of squared distances from each data
point to its closest centroid without using any loops.
"""
import numpy as np
...
def variance(X, C):
    ...
    Returns:
        numpy.float64: The total intra-cluster variance, or None on failure.
    """
    ...
    try:
        ...
        # Для каждой точки находим минимальное расстояние до центроида
        min_distances = np.min(distances, axis=1)

        # Общая внутрикластерная дисперсия — это сумма минимальных расстояний
        total_variance = np.sum(min_distances)

        # Возвращаем тип np.float64, у которого есть встроенный метод .round()
